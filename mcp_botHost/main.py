...
async def run_weather_agent(query: str, streaming: bool = True) -> None:
    ...
    try:
        ...
        filesystemServer = MCPServerStdio(
            name="Filesystem Server, via npx",
            params={
                "command": "npx",
                "args": ["-y", "@modelcontextprotocol/server-filesystem", "C:\\workspace\\MCP\\mcp_botHost"],
            },
        )
        ...
        try:
            ...
            weather_agent = Agent(
                ...
                # 指定 MCP 服务
                mcp_servers=[weather_server, filesystemServer],
                # 配置模型参数
        ...
        if weather_server:
            ...
            try:
                ...
                await filesystemServer.cleanup()
                print("MCP服务器资源清理成功！")
            except Exception as e:
